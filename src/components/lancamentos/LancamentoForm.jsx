      ...
      <div className="space-y-6">
        ...
        {contratoId && (
          <div className="space-y-2">
            ...
            <Popover>
              <PopoverTrigger asChild><Button variant="outline" className="w-full justify-between">{selectedItems.length === 0 ? "Selecione os itens..." : `${selectedItems.length} selecionado(s)`}</Button></PopoverTrigger>
              <PopoverContent className="w-[400px] p-2 bg-white shadow-xl"><div className="space-y-2 max-h-60 overflow-y-auto">{itensContratoAtivos.map(item => (<div key={item.id} className="flex items-center space-x-3 p-1 hover:bg-gray-50"><Checkbox id={`it-${item.id}`} checked={selectedItems.includes(String(item.id))} onCheckedChange={() => toggleItem(item.id)} /><label htmlFor={`it-${item.id}`} className="text-sm font-medium uppercase">{item.nome}</label></div>))}</div></PopoverContent>
            </Popover>
          </div>
        )}
        ...
        {selectedItems.map(itemId => {
          ...
          return (
            <div key={itemId} className="border rounded-lg p-5 bg-white space-y-4 shadow-sm">
              <div className="font-bold text-sm uppercase text-[#1a2e4a] border-b pb-2 flex justify-between">
                <span>{item?.nome}</span>
                {empenhos.find(e => String(e.item_contrato_id) === String(itemId)) && <Badge variant="secondary">Empenhado</Badge>}
              </div>
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-1"><Label className="text-xs">Nº NF *</Label><Input value={data.numero_nf || ""} onChange={e => setNfsData({...nfsData, [itemId]: {...data, numero_nf: e.target.value}})} /></div>
                <div className="space-y-1"><Label className="text-xs">Data NF *</Label><Input type="date" value={data.data_nf || ""} onChange={e => setNfsData({...nfsData, [itemId]: {...data, data_nf: e.target.value}})} /></div>
                <div className="space-y-1"><Label className="text-xs">Valor NF *</Label><Input value={formatarMoeda(data.valor)} onChange={e => handleNFMoneyChange(itemId, "valor", e.target.value)} /></div>
              </div>
            </div>
          );
        })}
            ...
            {ordensServico.map((os, i) => (
              <div key={os.id} className="border rounded-lg p-5 bg-gray-50/30 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1"><Label className="text-xs">Valor OS *</Label><Input value={formatarMoeda(os.valor)} onChange={e => handleOSMoneyChange(i, e.target.value)} /></div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="flex justify-end gap-3 pt-6 border-t">
          ...
          <Button onClick={executeSave} disabled={saving} className="bg-[#1a2e4a] min-w-[140px]">
