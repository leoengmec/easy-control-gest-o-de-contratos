...
export default function ExtratoPagamentos() {
  ...
  // Inicia em 2025 para garantir que os dados históricos apareçam de imediato
  ...
  // Máscara de centavos conforme solicitado (1018621 -> 10.186,21)
  ...
  // Plotagem dos dados nos Cards Superiores
    ...
    <div className="space-y-8 font-sans pb-10">
      ...
      <Card className="bg-white border-none shadow-md ring-1 ring-black/5 p-6">
        <div className="grid grid-cols-1 md:grid-cols-6 gap-4 items-end">
          ...
          <div className="space-y-2"><Label className="font-black uppercase text-[#1a2e4a] text-xs">Vigência/Aditivo</Label>
            <Select value={filtroVigencia} onValueChange={setFiltroVigencia}>
              <SelectTrigger className="h-12 bg-gray-50 border-gray-100 font-bold text-sm"><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="todos">Todos</SelectItem>
                {aditivos.filter(a => filtroContrato === "todos" || a.contrato_id === filtroContrato).map(a => (
                  <SelectItem key={a.id} value={a.id}>{a.tipo} - {a.numero}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="space-y-2 w-32"><Label className="font-black uppercase text-[#1a2e4a] text-xs">Mês</Label>
        ...
        <Table>
          <TableHeader className="bg-[#1a2e4a]">
            <TableRow className="hover:bg-[#1a2e4a] border-none">
              ...
              <TableHead className="text-white font-black uppercase text-xs">Auditoria (Responsável)</TableHead>
          ...
          <TableBody>
            {dadosFiltrados.length === 0 ? (
              <TableRow><TableCell colSpan={5} className="text-center py-40 text-gray-300 font-black uppercase text-lg">Troque o filtro de Ano para visualizar o histórico</TableCell></TableRow>
            ) : (
              dadosFiltrados.map((l) => (
                <TableRow key={l.id} className="hover:bg-blue-50/40 border-b border-gray-100">
                  ...
                  <TableCell>
                    ...
                    <div className="text-sm font-bold text-gray-600 uppercase block truncate max-w-[250px]">{l.item_label}</div>
                  </TableCell>
                  <TableCell>
                    ...
                    <div className="text-[10px] text-gray-400 font-black mt-1 uppercase flex items-center gap-1"><History size={10}/> Alt: {l.data_ultima_alteracao_status ? new Date(l.data_ultima_alteracao_status).toLocaleDateString() : "Sem alteração"}</div>
                  </TableCell>
                  <TableCell>
                    <Popover>
                      <PopoverTrigger asChild>
                        <button className="focus:outline-none cursor-pointer">
                          <Badge className={`text-[11px] font-black uppercase px-5 py-2 shadow-lg transition-transform hover:scale-105 ${l.status === 'Pago' ? 'bg-green-600' : 'bg-[#1a2e4a]'}`}>
                            {l.status}
                          </Badge>
                        </button>
                      </PopoverTrigger>
